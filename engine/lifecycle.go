...
import (
	...
	"github.com/bangbaew/prisma-client-go/binaries"
	"github.com/bangbaew/prisma-client-go/binaries/platform"
...
func (e *QueryEngine) ensure() (string, error) {
	...
	if v := strings.TrimSpace(strings.Replace(string(out), "query-engine", "", 1)); binaries.EngineVersion != v {
		note := "Did you forget to run `go run github.com/bangbaew/prisma-client-go generate`?"
