...
generate so `go run github.com/bangbaew/prisma-client-go generate` and any other generate commands you run will get
executed.

Put this line into a Go file in your project, usually in `main.go`:

```go
//go:generate go run github.com/bangbaew/prisma-client-go generate
...
RUN go run github.com/bangbaew/prisma-client-go prefetch

COPY . ./

# generate the Prisma Client Go client
RUN go run github.com/bangbaew/prisma-client-go generate
